...
def build_scene_prompts() -> list:
    ...
    ART_STYLE = "ethereal abstract expressionist oil painting with visible, thick impasto brushstrokes and layered, translucent glazes"
    
    # 2. DEFINE THE WORLD (Stays the same, but broad enough for variety)
    WORLD_ENVIRONMENT = "a dreamlike, shifting landscape of floating geometric forms, soft mist, and ethereal light beams"
...
"""
    ...
    LOCKED_PREFIX = (
        ...
        f"Environment: {WORLD_ENVIRONMENT}. "
        f"Color palette: {PALETTE_ANCHOR}. "
        "High-end fine art, museum-grade aesthetic. "
    )
...
"""
    ...
    for i, slide in enumerate(expanded):
        ...
        full_prompt = (
            f"{LOCKED_PREFIX} "
            f"SUBJECT: A visual representation of '{slide['text']}'. "
            f"SCENE DESCRIPTION: {raw_bodies[i]}. "
            f"{LOCKED_SUFFIX}"
        )
...
def make_frame(t, scenes, image_paths):
    active_idx = min(find_active_scene(t), len(scenes) - 1)
    ...
    if active_idx > 0 and elapsed < CROSSFADE_DUR:
        ...
        prev_bg  = apply_ken_burns(prev_img, prev_dur + elapsed, prev_dur, beat_pulse, prev_dir)
...
if __name__ == "__main__":
    print("═" * 65)
    print("  AI LYRIC VIDEO GENERATOR — THEME-LOCKED EDITION")
    print("═" * 65)
    print(f"  --skip-llama      : {'YES' if SKIP_LLAMA else 'NO'}")
    print(f"  --use-cached-imgs : {'YES' if USE_CACHED_IMGS else 'NO'}")
    print(f"  Target scenes     : {TARGET_SCENES}")
    print(f"  Fal.ai model      : {FAL_MODEL} ({FLUX_STEPS} steps)")
    print(f"  Parallel workers  : {PARALLEL_IMAGES}")
    print("═" * 65 + "\n")

    t0 = time.time()

    scenes = build_scene_prompts()
    print(f"\n📋 Scenes: {len(scenes)}  [{time.time()-t0:.0f}s elapsed]")
    ...
    for p in image_paths:
        get_cached_image(p)
    print(f"   ✅ {len(IMAGE_CACHE)} images in RAM  [{time.time()-t0:.0f}s elapsed]")

    print(f"\n🎬 Rendering {TOTAL_DURATION:.1f}s @ {FPS}fps …")
    video = VideoClip(lambda t: make_frame(t, scenes, image_paths), duration=TOTAL_DURATION)
    ...
    total = time.time() - t0
    print(f"\n✨ Done in {total/60:.1f} min  →  {OUTPUT_PATH}")
